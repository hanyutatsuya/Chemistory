...
printf("\tKSNSK:\t%s\n", pszFormular);
        ...
        while (!nErrorOccured && 
            ...
            if (NULL == (void *) (pstItems = SplitIFBlockItems(pszIFBlock))) {
                printf("Invalid IF block items - \"%s\"\n", pszFormular);
                ...
                default:                /* 異常な比較演算子                 */
                    printf("Invalid operator in KSNSK.\n");
                    nErrorOccured = TRUE;
                    free((void *) pszFormular);
                    pszFormular = (char *) NULL;
                    break;
...
if (NULL != (void *) pszFormular) printf("\tExpand:\t%s\n", pszFormular);
