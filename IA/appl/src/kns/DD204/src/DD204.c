...
/*	2.00	2008/09/25		K.Ameno		   X-M対応初版 <-- */
	...
	memset(&aNa_code_A00,0,sizeof(aNa_code_A00));
	memset(&aK_code_A00,0,sizeof(aK_code_A00));
	memset(&aCl_code_A00,0,sizeof(aCl_code_A00));
...
/*	2.00	2008/09/25		K.Ameno		   X-M対応初版 <-- */
	...
	ZbmlPrmGetVal(bp, "NA_CODE_A00", aNa_code_A00);
	ZbmlPrmGetVal(bp, "K_CODE_A00", aK_code_A00);
	ZbmlPrmGetVal(bp, "CL_CODE_A00", aCl_code_A00);

	ZbmlPrmGetVal(bp, "START_CTRLSEQ_DEF" , cNum);
	iStartCtrlSeq = atoi( cNum );
...
/* ] end 072/073 060417 */
	...
	memset( cWrkCd,0,sizeof(cWrkCd) );
	if( iType == ITEM_NA ) {
		GetKmkcd( 0,aNa_code_A00,cWrkCd );
	} else if ( iType == ITEM_K ) {
		GetKmkcd( 0,aK_code_A00,cWrkCd );
	} else {
		GetKmkcd( 0,aCl_code_A00,cWrkCd );
	}
	strcpy( p->pKekka[0].kmkcd, cWrkCd );
	/* No. 5 検査グループ設定 */
	/* No. 6 項目コード設定 */
	...
	/* No.31 内部項目コード設定 尿用内部項目を設定 */
	if( iType == ITEM_NA ) {
		strcpy( p->pKekka[0].nbkmkcd,aNa_code_A00 );
	} else if ( iType == ITEM_K ) {
		strcpy( p->pKekka[0].nbkmkcd,aK_code_A00 );
	} else {
		strcpy( p->pKekka[0].nbkmkcd,aCl_code_A00 );
	}
	/* No.32 定性・定量フラグ設定 */
	/* No.51 検査結果１ */
	memset( cWrkCd,0,sizeof(cWrkCd) );
...
//	iWrk = iRcvMsgCnt;
	...
	/* 内部項目コード設定 */
	if(!strcmp(sResultRcd.cAppCode, aNa_code)) {
		strcpy( sResultRcd.cAppCode, aNa_code_A00 );
	} else if(!strcmp(sResultRcd.cAppCode, aK_code)) {
		strcpy( sResultRcd.cAppCode, aK_code_A00 );
	} else if(!strcmp(sResultRcd.cAppCode, aCl_code)) {
		strcpy( sResultRcd.cAppCode, aCl_code_A00 );
	}
...
int ChkTestCode(SOCKET_INF* p,char* aRcvMsg)
{
	...
	/* 尿用 内部項目コード設定 */
	if(!strcmp(sCalbRcd.cTestCode, aNa_code)) {
		strcpy( sCalbRcd.cTestCode, aNa_code_A00 );
	} else if(!strcmp( sCalbRcd.cTestCode, aK_code)) {
		strcpy( sCalbRcd.cTestCode, aK_code_A00 );
	} else if(!strcmp( sCalbRcd.cTestCode, aCl_code)) {
		strcpy( sCalbRcd.cTestCode, aCl_code_A00 );
	}
...
/*	2.00	2008/09/25		K.Ameno		   X-M対応初版 <-- */
			...
			if( iWrk == 0 ) {
				for( iCnt=0; iCnt<p->iOrderNum; iCnt++ ) {
					...
					/* 尿用の内部項目コードを分析機向けコードに変換 */
					if( !strcmp( p->sSndorder.kmkcd[iCnt],aNa_code_A00 ) ) {
						strcpy( p->sSndorder.kmkcd[iCnt],aNa_code );
					} else if( !strcmp( p->sSndorder.kmkcd[iCnt],aK_code_A00 ) ) {
						strcpy( p->sSndorder.kmkcd[iCnt],aK_code );
					} else if( !strcmp( p->sSndorder.kmkcd[iCnt],aCl_code_A00 ) ) {
						strcpy( p->sSndorder.kmkcd[iCnt],aCl_code );
					}
...
_sFncSyslog (LOG_TRCKIND piPri, char * pcFncname,char * pcMsg ,char * pcErrCode, char * pcExeFlg, char * file, long lineno) 
{
	...
	strncpy(plm.pcAppName, "DD204", 32);
