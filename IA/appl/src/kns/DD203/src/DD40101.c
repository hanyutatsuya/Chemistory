...
#ifdef _DEBUG_
			printf( "\n● RealTimeChk(%d) : p->pKekka[li_idxC].knssu = %d\n", __LINE__, p->pKekka[li_idxC].knssu );
			printf( "\n● RealTimeChk(%d) : iKnssu                   = %d\n", __LINE__, iKnssu );
#endif
...
#endif
    printf( "SetKekkaData 前 li_idxO %d\n", li_idxO );
			SetKekkaData ( p, li_idxO ) ;
    printf( "SetKekkaData 後 li_idxO %d\n", li_idxO );
	...
	for (paraCnt = 0; kmkcd[paraCnt*7] != 0; paraCnt++ ) {
		...
		for (iCnt = 0; iCnt < KekkaCnt; iCnt++) {
			...
			if(  strncmp(pKekka[iCnt].kmkcd, &kmkcd[paraCnt*7], 7) == 0 ) {
				...
				// 関連項目は片方が用手ならば、もう片方も用手にしなければ後工程でおかしくなる。
				// 関連元が用手再検の場合は関連先も用手再検にする。
