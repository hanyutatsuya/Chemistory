...
# IA 上 ( socket 経由でIA上のshellを起動.backup->del）
sh $SHDIR/rsh_back_del.sh &

# プロセス終了処理後にまだ残っているＤＢ接続があった場合は強制終了する。
# (画面等の終了忘れ対策。)
db2 terminate > /dev/null

sh $SHDIR/db_force.sh >> $LOGFILE

# DB2 CONNECT
db2 connect to $DB user $USER using $PASS > /dev/null
if [ $? -gt 0 ]; then
	echo `date`" ERROR : ＤＢ接続(２回目)に失敗しました" >> $ERR_LOG_DIR/$NAME_ERRLOG"daily.log"
	errMsg
fi
...
# Z 上
#sh $SHDIR/data_back.sh $FBUDIR >> $LOGFILE
echo "[`date`][data_back] start ----->" >> $LOGFILE
sh $SHDIR/data_back.sh $FBUDIR
...
# ログを出力しないように修正
#sh $SHDIR/kekkaout_del.sh $FBUDIR >> $LOGFILE
...
#######################################################################
# No.16 累積の保存期間(100日、730日)より前のデータ削除
#         100日分保存と730日分(2年間)保存に分けて削除する
#######################################################################

startMsg "累積の保存期間(100日、730日)より前のデータ削除" $LOGFILE

# CHANGE DIRECTRY 
cd $EXPDIR

# 累積【IA側の100日分保存】削除用シェル
sh $SHDIR/del_rui_100.sh $DB $USER $PASS >> $LOGFILE

# 累積【IA側の730日分(2年間)保存】削除用シェル
sh $SHDIR/del_ruishu_730.sh $DB $USER $PASS >> $LOGFILE

endMsg "累積の保存期間(100日、730日)より前のデータ削除" $LOGFILE

#######################################################################
# No.17 累積・集計テーブルrunstats＆未報告分データの削除
#######################################################################

startMsg "累積集計情報runstats" $LOGFILE

# 履歴テーブルにrunstatsをかける
sh $SHDIR/runstats_rireki.sh $DB $USER $PASS >> $LOGFILE

# 累計、集計テーブルにrunstatsをかける(z990)
sh $SHDIR/runstats_ruishu.sh $DB $USER $PASS >> $LOGFILE
# 累計、集計テーブルにrunstatsをかける(IA) → 週次でcronから起動に変更
#sh $SHDIR/runstats_ruishu.sh KEA00DBN $USER $PASS >> $LOGFILE

endMsg "累積集計情報runstats" $LOGFILE
