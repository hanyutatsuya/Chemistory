...
#	db2 "import from ./$TBL.ixf of ixf allow write access commitcount 5000 messages ./RUI_$TBL.msg INSERT_UPDATE into RUI_$TBL"
#	if [ $? -gt 0 ]; then
...
do
	echo "[`date`][RUI_$TBL] import start ----->"
	db2 "import from ./$TBL.ixf of ixf allow write access commitcount 5000 messages ./RUI_$TBL.msg INSERT into RUI_$TBL"
	if [ $? -gt 0 ]; then
		echo `date`" ERROR : $TBL ‚ÌIMPORT‚ÉŽ¸”s‚µ‚Ü‚µ‚½" >> $LOG_DIR/$NAME_ERRLOG"imp_rui.log"
		db2 terminate
		rm *.$$
