 ...
 * --------------------------------------------------------------------
 *	0.01	 2006/01/12 	S.Tsunekawa    Create
 *	1.00	 2006/09/22 	Nagata		   var1.0 リリース
 *	1.10	 2006/11/21 	Nagata 
 *							定性値の基準値判定機能を追加
 *	1.11	 2006/12/16 	YOON
 *							SQL 重複とロックエラー追加
 * --------------------------------------------------------------------
 *	Ver.	 Date			Name		   History
 * --------------------------------------------------------------------
 *	2.00	2008/09/18		K.Ameno			X-M対応初版
 *	2.01	2009/09/29		K.Ameno			障害[0909200955]対応
 *	2.02	2010/01/25		H.Sekiya		パニックチェック修正
 *	2.03	2014/01/15		H.Sekiya		同一依頼、別検体の測定済み結果
											を取り込まない。
 **********************************************************************/
...
/* ADD 2006/08/14 S.Ueda 照合チェック(rawdataがブランクの場合NG) */
...
/* RIBINDシェルパス ADD 2006-09-06 H.Haga */
char		gcMyRebindShell[256];

/* ADD 2006-11-21 Nagata 定性値基準値判定の為の換算テーブル */
...
/* 計算式拡張対応
 * 相関式中の計算式にのみ適用、計算項目の計算は、 DD401 では行わない。
 */
...
int main(int argc, char **argv)
{
	...
	strcpy(GetFolderIndex, GetFolder);
	strcat(GetFolderIndex, "../Index/");
...
/* ADD 2006/08/14 S.Ueda 乳び・溶血チェック << */
	...
	/* REBINDシェル取得 */
	ZbmlPrmGetVal(bp, MYAPP_PRM_REBIND_SHELL, gcMyRebindShell);
...
/* ADD 2006-11-23 Nagata 異常値マーク代入コメント >> */
...
/* ADD 2006-11-21 Nagata 異常値マーク代入コメント << */
	ZbmlPrmClose(bp);
...
{
	DIR 			*dirp;
	struct dirent	**entp; 	/* ADD 2006-08-31 S.Ueda */
	struct flock	fl;
	FILE *			fd;
	char			*fname;
	char			*fname2;
	...
	int 			iKkaCnt;
	Kekka			strKekka[100];
	int 			num_file, n;	/* ADD 2006-08-31 S.Ueda */
	...
	for (;;) {
		...
		/* 受信ファイルがあるか確認 */
/* UPD 2006-08-31 S.Ueda >> */
		num_file = scandir( GetFolderIndex, &entp, 0 , alphasort );
		for( n = 0; n < num_file; n++ ) {
			/* ディレクトリは除外する */
			if ( (strcmp(entp[n]->d_name, ".") == 0) ||
				 (strcmp(entp[n]->d_name, "..") == 0) ||
				 (strcmp(entp[n]->d_name, "out") == 0)) {
				free( entp[n] );
				continue;
			}
			/***
...
/* UPD 2006-08-31 S.Ueda << */
			...
			strncpy(fname2, GetFolder, 256);
			strncat(fname2, "out/", 256);
/* UPD 2006-08-31 S.Ueda >> */
...
/* UPD 2006-08-31 S.Ueda << */
			...
			while (fgets(buff, 1024, fd) != NULL) {
				/* 読み出したファイルをDBへ登録 */
				if (flgCtrl == 0) {
					/* CHG E.Osada 2006-08-15 結果登録処理はファイル読込み後に行うようにする */
			...
			/* ADD E.Osada 2006-08-18 血清情報対応処理追加 */
			if( (flgCtrl == 0) && (err_flg == 0) ) {
			...
			/* Indexファイルの削除 */
			strncpy(fname, GetFolderIndex, 256);
			strncat(fname, entp[n]->d_name, 256);
			remove(fname);
			free( entp[n] );
		}
...
int EntryKekka(Kekka *pSrc, int iKcnt)
{
	...
	int 		iSaikenFlg;
	int 		iKesseiflg;
	int 		iDenkaiflg; 	/* ADD 2006-09-01 Nagata */
	short		rKnssu; 		/* ADD 2006-08-21 S.Ueda */
	long		num_irai;		/* ADD 2006-08-31 S.Ueda */
	char		errMsg[1024] = {0,};
	int			iFukusuflg = 0; 
	/* ADD E.Osada 2006-08-17 BMでの電解質電文の扱い変更 */
...
/* ADD 2006-08-31 S.Ueda >> */
			...
			{
				/* 2006-12-13	YOON */
				sprintf(errMsg,"SQLCODE:%ld Input knskirymd(%s) kntno(%s) knsgrp(%s)"
	...
	for( i=0; i < iKcnt ;++i ) {
		...
		if (pKekka->knsflg != 'H') {
			...
			if (pKekka->irikbn != 'Y') {	/* キャリブレーション以外のとき */
				...
				if (ret < 0){
					sprintf(errMsg,"SQLCODE:%ld ZdbKekkaSelectTnyrkCount",ret);
					/* 2006-12-13	YOON */
					if(ret == SQL_ERR_DUPLICATION || ret == SQL_ERR_LOCK){
						sFncPutLog (TRCKIND_WARNING, __FILE__, "EntryKekka",__LINE__,
							EXEFLG_NON, MYAPP_LOG_ERRCODE_NON, errMsg) ;
				...
				if (ret < 0){
					sprintf(errMsg,"SQLCODE:%ld ZdbKekkaSelectFukusuCount",ret);
					if(ret == SQL_ERR_DUPLICATION || ret == SQL_ERR_LOCK){
						sFncPutLog (TRCKIND_WARNING, __FILE__, "EntryKekka",__LINE__,
							EXEFLG_NON, MYAPP_LOG_ERRCODE_NON, errMsg) ;
						return(RET_CONTINUITY);
				...
				if (pKekka->knsflg != 'M') {
					...
					{
						sprintf(errMsg,"SQLCODE:%ld ZdbKmkmstSelectKey",ret);
						/* 2006-12-13	YOON */
						if(ret == SQL_ERR_DUPLICATION || ret == SQL_ERR_LOCK){
							sFncPutLog (TRCKIND_WARNING, __FILE__, "EntryKekka",__LINE__,
								EXEFLG_NON, MYAPP_LOG_ERRCODE_NON, errMsg) ;
...
/*	2.01	2009/09/29		K.Ameno		   障害[0909200955]対応 --> */
/*** 関数の実行結果がOKでなかった場合は、固定でSTOP応答していた   ***/
/*** ものを  関数の実行結果を上位に応答するように変更              ***/
/*						return(RET_STOP);*/
						return(ret);
/*	2.01	2009/09/29		K.Ameno		   障害[0909200955]対応 <-- */
					}
	...
	for( i=0; i < iKcnt ; ++i ) {
		...
		if (pKekka->knsflg != 'H') {
			...
			/*	存在しない */
			/*		自検査工程区分 "K" ： 自動再検 */
			/* CHG E.Osada 2006-08-06 ret をここで使うとNG別変数に格納する */
			memset(&wOrder, 0, sizeof(wOrder));
			if( (ret1 = getOdrLast( pKekka, &wOrder )) < -1 ) {
				/* ＤＢエラー(ログ) */
				return -1;
			}
			if( ((ret1 == 0) && (pKekka->ksktkbn == 'K')) ) {
				/* 自動再検 */
				iSaikenFlg = 0;
			}
			else {
...
/* ADD 2006-08-31 S.Ueda << */
			...
			do {
				...
				/* Nagata -803 → SQL_ERR_DUPLICATION へ変更 */
...
/* ADD 2006-08-31 S.Ueda >> */
		...
		/* 再流しの処理を行わない。自分が生きる */
						...
						if(num_irai < 0)
						{
							/* 2006-12-13	YOON */
							/* Nagata 未報告完了レコードの検索 */
							sprintf(errMsg, "SQL CODE:%d ZdbSelectMihoKekka",num_irai);
							...
							/* 再流し前の検査回数がすべて中間 */
							/* ADD 2007-05-21 Nagata フラグ類を再チェックして2以上が存在しない場合 */
		...
		if (pKekka->knsflg != 'H' && iFukusuflg != 1) {
			if (pKekka->irikbn != 'Y') {	/* キャリブレーション以外のとき */
				...
				if(ret<0)
				{
					...
					if(pKekka->knsflg == 'M'){
						/* CHG Nagata 2006-08-15 再流しの場合は依頼の検査回数に＋１するのみ */
						pIrai->knssu++;
					}
					else{
						/* CHG 2006-09-16 */
						/* ODRSSLF(オーダ作成フラグ)が'1'場合は誰かがWS指示したのでここでは */
						/* なにもしない */
						...
						if( pIrai->odrssflg == '1' )
						{
							...
							/* CHG Nagata 2006-08-14 依頼の検査回数を結果に合わせる */
							/* CHK Nagata 2006-09-27 依頼の検査回数と(結果の検査回数 + 1)が同一だった場合、 */
							/*						 更新を行わない。 検査フラグを"A"に更新する */
							/* CHG Nagata 2006-11-13 依頼の検査回数≧（結果の検査回数＋１）だった場合、 */
							/*						 再検を出さない */
...
/*	2.00	2008/09/18		K.Ameno		   X-M対応初版 --> */
/*** X-M結果を基準値マスタの小数点桁数で丸める ***/
/*   → 丸めちゃだめ。↓↓↓はコメントにし、sstn = 6固定にする(オンラインで小数点以下6桁で出力している為) */
...
//		strncpy(sKjnmst.kmkcd, pFld[5], 7);
//
//		ret = ZdbKjnmstSelectKey3(&sKjnmst, &pKjnmst, &cntKjnmst);
//		if (ret != HRC_SQL_NORMAL) {
//			sstn = 4; /* 基準値マスタで設定する最大桁数 */
//		} else {
//			sstn = pKjnmst->kjsstnksu;
//			ZbmlFree(pKjnmst);
//		}
//	}
/*	2.00	2008/09/18		K.Ameno		   X-M対応初版 <-- */
...
/*** X-M結果を基準値マスタの小数点桁数で丸める ***/
	...
	if (sstn == 9) {
		sprintf(tmp, "%.0f", (atof(pFld[15]) + 0.00001));
	} else {
//		sprintf(tmp, "%.*f", sstn, (atof(pFld[15]) + 0.00001));
...
/*** X-M確定フラグは人が確定した場合のみ1とする ***/  
	pXbarm->xbarmjkflg = '0'; /* Ver2.00 '1' -> '0'に変更 */
/*	2.00	2008/09/18		K.Ameno		   X-M対応初版 <-- */
...
int 		shm_Key ;
{
	...
	if (db_ret == 0L) {
		sprintf(gMsgBuff,"DB Connect OK [%s]\n", dbname);
		sFncSyslog(TRCKIND_TRACE, "sFncRebind", gMsgBuff);
	}
...
_sFncSyslog (int piPri, char * pcFncname,char * pcMsg ,char * pcErrCode, char * pcExeFlg, char * file, long lineno)
{
	...
	strncpy(plm.pcAppName, "DD401", 32);
...
#ifdef KKAFLTMST
#ifdef DEBUG2
	LogMsg("新しい追加コード filterChk2");
#endif
	ret = filterChk2(pKekka, pKjnmst);
#else
	ret = filterChk(pKekka, pKjnmst);
#endif
...
{
	Kkafltmst	*pKkafltmst;
	Kkafltmst	sKkafltmst;
	int 		ret;
	int 		Kkafltmstflg;
	int 		cnt;
	int 		lCnt;
	...
	ret = ZdbKkafltmstSelectGRPKMKKey(&sKkafltmst, &pKkafltmst, &cnt);
	if ((ret != HRC_SQL_NORMAL) && (ret != HRC_SQL_NOT_FOUND)) {
		sprintf(errMsg,"SQL CODE:%d ZdbKkafltmstSelectGRPKMKKey",ret);
		if(ret == SQL_ERR_DUPLICATION || ret == SQL_ERR_LOCK){
			sFncPutLog (TRCKIND_WARNING , __FILE__, "filterChk", __LINE__,
					 EXEFLG_NON, MYAPP_LOG_ERRCODE_NON, errMsg) ;
			return(RET_CONTINUITY);
		}
		sFncSyslog ( LOG_ERR, "filterChk()", errMsg ) ;
		return(RET_STOP);
	}
...
/* 計算式から項目コードを探し出す。 */
int chkKoumoku(char *src, char koumoku[][8]) {
	int lcnt;
	int retcnt;
	int cnt;

	retcnt = 0;
	for (lcnt = 0; src[lcnt] != '\0'; lcnt ++) {
		if (src[lcnt] == '[') {
			lcnt++;
			cnt = 0;
			while ((src[lcnt] != ']') && (src[lcnt] != '\0')) {
				koumoku[retcnt][cnt] = src[lcnt];
				cnt ++;
				lcnt++;
			}
			koumoku[retcnt][cnt] = '\0';
			retcnt ++;
		}
	}
	return retcnt;
...
/* 計算式中の項目コードを置き換える */
void chgKoumoku(char * src, char *koumoku, char *atai)
{
	char	buff[16];
	char	*pichi;
	int 	ichi;
	...
	sprintf(buff, "[%s]", koumoku);
	pichi = strstr(src, buff);
	if (pichi == NULL) {
		return;
	}
	ichi = (int)(pichi - src);
	...
	alen = strlen(atai);
	blen = strlen(buff);
	memmove(src+ichi+alen, src+ichi+blen, strlen(src)-(ichi+blen)+1);
	memcpy(src+ichi, atai, alen);
}

int keisanChk(Kekka *pKekka)
{
	Calckmkmst	sCalckmkmst;
	Calckmkmst	*pCalckmkmst;
	int 		cntCalckmkmst;
	Kekka		sKekka;
	Kekka		*spKekka;
	int 		cntKekka;
	...
	memset(&sCalckmkmst, 0, sizeof(sCalckmkmst));
	strncpy(sCalckmkmst.knsgrp, pKekka->knsgrp, 16);
	trim(tmpBuff, pKekka->kmkcd);
	sprintf(chkBuff, "%%[%s]%%", tmpBuff);
	strncpy(sCalckmkmst.ksnsk, chkBuff, 128);

	/* 現在の項目を使っている計算式を探し出す。 */
	ret = ZdbCalckmkmstSelectKey2(&sCalckmkmst, &pCalckmkmst, &cntCalckmkmst);
	if ((ret != HRC_SQL_NORMAL) && (ret != HRC_SQL_NOT_FOUND)) {
		sprintf(errMsg,"SQL CODE:%d ZdbCalckmkmstSelectKey2",ret);
		if(ret == SQL_ERR_DUPLICATION || ret == SQL_ERR_LOCK){
			sFncPutLog (TRCKIND_WARNING , __FILE__, "rirekiChk", __LINE__,
				 EXEFLG_NON, MYAPP_LOG_ERRCODE_NON, errMsg) ;
			return(RET_CONTINUITY);
		}
		sFncSyslog ( LOG_ERR, "keisanChk()", errMsg ) ;
		return(RET_STOP);

	}

	if (ret != HRC_SQL_NOT_FOUND) {
		for (lcnt = 0; lcnt < cntCalckmkmst; lcnt++) {
			trim(calBuff, pCalckmkmst[lcnt].ksnsk);
			/* 計算項目から項目コードを取り出し */
			char	koumokuBuff[16][8];
			int 	koumokuIchi[16];
			int 	koumokuCnt;
			int 	cnt;
			char	rawData[24];
			...
			strncpy(sKenorder.kntno, pKekka->kntno, 13);
			strncpy(sKenorder.knsgrp, pKekka->knsgrp, 16);
			strncpy(sKenorder.kmkcd, pCalckmkmst[lcnt].kmkcd, 7);
			sKenorder.knssu = pKekka->knssu;
			sKenorder.knssueda = 0;
			ret = ZdbKenorderSelectKeyCount(&sKenorder);
			if ( ret == 0 ) {
				/* ｵｰﾀﾞｰが存在しないので、計算を行わない */
				continue;
			}

			koumokuCnt = chkKoumoku(calBuff, koumokuBuff);

			chkFlg = 0;
			for (cnt = 0; cnt < koumokuCnt; cnt++) {
				trim(tmpBuff, pKekka->kmkcd);
				if ((strncmp(tmpBuff, koumokuBuff[cnt], 7) == 0) &&
					(pKekka->kkaysnflg == '1')) {
					/* 現在の項目で優先フラグがついているとき、検査結果の時 */
					trim(rawData, pKekka->rawdata);
				}
				else {
					/* 結果情報の検索 */
					memcpy(&sKekka, pKekka, sizeof(Kekka) );
					strncpy(sKekka.kmkcd, koumokuBuff[cnt], 7);

					ret = ZdbKekkaSelectKey2(&sKekka, &spKekka, &cntKekka);
					if ((ret != HRC_SQL_NORMAL) && (ret != HRC_SQL_NOT_FOUND)) {
						ZbmlFree(pCalckmkmst);
					...
					if (ret == HRC_SQL_NOT_FOUND) {
						/* 検査結果が見つからないので、計算をしない。 */
						chkFlg = 1;
						break;
					}
					trim(rawData, spKekka->rawdata);
					ZbmlFree(spKekka);
				}
				/* 計算式の計算項目の置き換え */
				chgKoumoku(calBuff, koumokuBuff[cnt], rawData);
			}
			if (chkFlg == 1) {
				/* 項目が全てそろっていないので、処理を中断。 */
				continue;
			}
			/* 文字列計算式を計算する。 */
			calkekka = StringCal(calBuff);

			/* 計算した結果を登録 */
			/* 最初に既にDBに登録済みかチェックする。 */
			memcpy(&sKekka, pKekka, sizeof(Kekka) );
			strncpy(sKekka.kmkcd, pCalckmkmst[lcnt].kmkcd, 7);
			sKekka.knssueda = 0;
			...
			if (ret != HRC_SQL_NOT_FOUND) {
				/* 既に登録済みなのでアップデートする。 */
				sprintf(buff, "%f", calkekka);
				strncpy(spKekka->rawdata, buff, 20);
				...
				memset(&sKjnmst, 0, sizeof(sKjnmst));
				strncpy(sKjnmst.kmkcd, spKekka->kmkcd, 7);
				strncpy(sKjnmst.kjno, spKekka->kjno, 9);
				...
				if (ret != HRC_SQL_NOT_FOUND) {
					filterChk(spKekka, pKjnmst);
					kjnChk(spKekka, pKjnmst);
					ZbmlFree(pKjnmst);
				}
				...
				/* 新しく作成された、結果レコードの登録チェックを行う。 */
				TourokuChk(spKekka, &ret);
	...
	if (ret != HRC_SQL_NOT_FOUND) {
		...
		if (ret != HRC_SQL_NOT_FOUND){
			...
			if (flg == 1) {
				...
				/* 分画親の結果レコードを作成する。 */
				/* ADD 2007-05-16 項目コード '0006485'の分画親レコードはインサートしない */
				/* UPD 2007-06-04 項目コード '0006485'は分画親の特別処理を一切行わない */
...
int shougoChk(Kekka *pKekka, Kenorder *pKenorder, int *iKesseiflg)
{
	...
	int 		e_hit;
	char		zen_kyoyou;
	char		sKisya[128];	/* ADD E.Osada 2006-07-21 */
	int 		iKisya; 		/* ADD E.Osada 2006-07-21 */
	char		errMsg[1024] = {0,};
	 ...
	 ***/
	/* 検査回数をデクリメント ADD 2006/08/03 OSADA */
	sKekka.knssu -= 1;
	...
	if (ret == HRC_SQL_NOT_FOUND || spKekka->knsflg == 'H') {
		/* ADD 2006-09-01 Nagata データが取得できないとき(再流し等イレギュラーケース対応） */
		/* ADD Nagata 2006-10-05 */
	...
	if (ret != HRC_SQL_NOT_FOUND) {
		...
		if( cFlg ) {
			...
			if( ((rawData >= 0) || (rawDataZen >= 0)) && (db_hrng >= chk) &&
				...
				/* ADD E.Osada 2006-08-19 比色がきていない(電解質のみ)場合は 'H' */
				if( *iKesseiflg ) {
				...
				pKekka->syougoflg = '2';
				/* 照合NGは再検対象とする 2006-07-08 E.Osada */
				pKekka->knsflg	= 'R';				// 検査フラグ
				spKekka->syougoflg = '2';
				/* 照合NGは再検対象とする 2006-07-08 E.Osada */
				spKekka->knsflg = 'R';				// 検査フラグ
		...
		/* 2006-09-08 Nagata レンジチェックにレコード自体存在しない場合の対応 */
		if (flg == 1) { 					/* 再検優先 */
			pKekka->kkajkcd 		= '3';				/* 結果状況コード */
			pKekka->kkaysnflg		= '1';				/* 結果優先フラグ */
			pKekka->siyflg			= '1';				/* 採用フラグ */
	...
	for( iCnt=0; iCnt < gKrkCnt ;++iCnt ) {
		...
		for( i=0; i < iTcnt ;++i ) {
			...
			if( iKhit ) {
				...
				if( iKcnt )
				{
					/* 優先結果が報告済み、保留、未報告完了のいずれかの場合、優先結果のフラグのUPDATEは行わない */
					if( pKkaYkrn->knsflg != 'E' && pKkaYkrn->knsflg != 'H' && pKkaYkrn->knsflg != 'M' ) {
						/* 2.検査ＦＬＧセット */
...
/* ADD E.Osada 2006-07-16 レンジチェック追加 */
/*  YOON 利用する部分がなし。 */
int lngChk(Kekka *pKekka, Kenorder *pKenorder)
{
	...
	sRngchkmst.sbt = pKenorder->sbtkbn;
	ret = ZdbRngchkmstSelectKey2(&sRngchkmst, &pRngchkmst, &cnt);
	if ((ret != HRC_SQL_NORMAL) && (ret != HRC_SQL_NOT_FOUND)) {
		sprintf(errMsg,"SQL CODE:%d ZdbRngchkmstSelectKey2",ret);
		if(ret == SQL_ERR_DUPLICATION || ret == SQL_ERR_LOCK){
			sFncPutLog (TRCKIND_WARNING, __FILE__, "lngChk",__LINE__,
					EXEFLG_NON, MYAPP_LOG_ERRCODE_NON, errMsg) ;
			return(RET_CONTINUITY);
		}
		sFncSyslog ( LOG_ERR, "lngChk()", errMsg) ;
		return(RET_STOP);
	}
	else if (ret == HRC_SQL_NOT_FOUND) {
		strncpy(sRngchkmst.sstcd, "0000000", 7);
	...
	if (ret != HRC_SQL_NOT_FOUND) {
		int 	lCnt;
		for (lCnt = 0; lCnt < cnt; lCnt++) {
			if (pRngchkmst[lCnt].hnikbn == '1') {		/* L */
				if( (atof(pRngchkmst[lCnt].lrng) <= rawData) &&
							(atof(pRngchkmst[lCnt].hrng) >= rawData) ) {
					/* 正常 */
					pKekka->renjicflg = '1';
					break;
				} else if( (pRngchkmst[lCnt].ssa == '1') &&
							(atof(pRngchkmst[lCnt].hrng) < rawData) ) {
					/* 高値でエラー */
					pKekka->renjicflg = '3';
					break;
				} else if( (pRngchkmst[lCnt].ssa == '2') &&
							(atof(pRngchkmst[lCnt].lrng) > rawData) ) {
					/* 低値でエラー */
					pKekka->renjicflg = '4';
					break;
				}
...
int getOdrLast(Kekka *pKekka, Kenorder *p_Order)
{
	...
	if ((lRet != HRC_SQL_NORMAL) && (lRet != HRC_SQL_NOT_FOUND)) {
		sprintf(errMsg,"SQL CODE:%d ZdbKenorderSelectKey",lRet);
...
/*	 戻値  TRUE   OK */
/*		   FALSE  NG */
/* ADD 2006-09-29 Nagata 初回、再検、どちらかのrawデータが */
/*							マイナスだったら照合NGとする */
...
int getKekkaData(char * buff, Kekka *pKekka)
{
	...
	strncpy( pKekka->jisksbr, pFld[42], sizeof( pKekka->jisksbr )-1);
	...
	/* CHG E.Osada 2006-07-14  RAWDATA を左詰めで登録する */
...
static int	NyubiYoketuChk(Kekka * pKekka, int KekkaCnt, int lCnt, int fNyuYou)
{
	...
	if (ret == HRC_SQL_NOT_FOUND) {
		...
		/* UPD 2006-09-05  再流しの際にリアルチェックが取得出来ない場合があるため、 */
		...
		case '1':	/* 乳びのみ */
			if (fNyuYou & 1) {
				...
				if( (pKekka[lCnt].kkacmt1[0] == ' ') || (pKekka[lCnt].kkacmt1[0] == '\0') ) {
					...
					/* ADD 2006-09-14 Nagata もし、結果コメントに乳びコメントが入っていたらなにもしない */
				}
				else {
		...
		case '2':	/* 溶血のみ */
			if (fNyuYou & 2) {
				...
				if( (pKekka[lCnt].kkacmt1[0] == ' ') || (pKekka[lCnt].kkacmt1[0] == '\0') ) {
					...
					/* ADD 2006-09-14 Nagata もし、結果コメントに溶血コメントが入っていたらなにもしない */
				}
				else {
					memset(pKekka[lCnt].kkacmt2, 0, sizeof(pKekka[lCnt].kkacmt2));
					strncpy( pKekka[lCnt].kkacmt2, pKekka[lCnt].kkacmt1, 5);
...
/* ADD 2006/08/14 S.Ueda 乳び・溶血チェック << */
/* ADD 2006/08/17 E.Osada */
int chkDenkai( Kekka *pKekka )
{
	char	sBuf[10];
	int 	i;
	int 		ret = 0;
	...
	/* BMだったら電解質モジュールかを確認する */
	/* LABOは全項目チェック対象 */
	if( !memcmp(pKekka->bskkbn, "BIOM", 4) ) {
		memset(sBuf1, 0, sizeof(sBuf1));
		trim(sBuf1, pKekka->bskgok);
		...
		/* 後ろ2文字が'00'以外なら対象 */
		if( (sBuf1[i] == '0') && (sBuf1[i-1] == 0) )
			return( 0 );
	}

	return( 1 ); /*  ADD Nagata 2006-09-01 */
}

/* ADD E.Osada 2006-08-18 血清情報処理追加 */
int kesseiCHK(Kekka *pSrc, int iKcnt)
{
	int 		i, KesFlg;
	...
	int 		ret;
	int 		num_spKekka;
	char		sBuff[10];	/* ADD E.Osada 2006-09-16 */
	char		errMsg[1024] = {0,};
