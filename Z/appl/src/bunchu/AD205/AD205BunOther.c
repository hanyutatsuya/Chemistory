...
/*  4.01    2009-02-10      Merits K.Ameno  緊急SUN EOSL対応        */
/********************************************************************/
...
int ABunGetKnsgrp( Bunchu *strBunchu, Kanjya *strKanjya, char *sKmkcd, char *sKnsgrp )
{
	...
	if( !memcmp(sBuf, "012", 3) || !memcmp(sBuf, "013", 3) || !memcmp(sBuf, "014", 3) ||
		...
		!memcmp(sBuf, "404", 3) ||
		!memcmp(sBuf, "405", 3) || !memcmp(sBuf, "412", 3) || !memcmp(sBuf, "490", 3) ) {
		iSflg = 1;
	}
	// 2006-05-10 仕様追加分
...
/*			(strBunchu->tknflg == ' ') && (strBunchu->jtflg == ' ') ) {*/
		if( !memcmp(strBunchu->smpcd, "0054", 4) && (strBunchu->tknflg == ' ') && (strBunchu->jtflg == ' ') ) {
...
/* 2009-02-20 緊急SUN EOSL対応 INS START */
	...
	if( !memcmp(pKan->cdkbn1, "17", 2) ) {
		...
		if( !memcmp(buf, "012",3) || !memcmp(buf, "013", 3) || !memcmp(buf, "014", 3) ||
			...
			!memcmp(buf, "404",3) ||
			!memcmp(buf, "405",3) || !memcmp(buf, "412", 3) || !memcmp(buf, "490", 3) ) {
			pOd->skflg = '1';
		}
	}
...
/*	if( A105trmstSelectKey(&strTr, &pTr, &num) < 0 ) {*/
	if( A205rkmstSelectKey(&strRk, &pRk, &num) < 0 ) {
/* 2009-02-20 緊急SUN EOSL対応 INS END */
...
/* 2009-02-20 緊急SUN EOSL対応 INS END */
	...
	// add 2007.05.31 waj 緊急依頼ロットNoを依頼から取得・設定
	pOd->kkirilotno = pIrai->kkirilotno;
...
*/
	...
	//1.指定施設コードで読込み
	if( (ret = A205rngchkSelKey(&strRng, &pRng, &num)) < 0 ) {
...
//	strcpy(pRe.renjicpara, "1000,500");
	...
	if( AD205sknchkSelKey(&strSkn, &pSkn, &num) < 0 ) {
...
*/
	...
	if( A205krkmkSelKey(&strKnr, &pKnr, &num) < 0 ) {
...
//	if( ZdbKmkmstSelectKey(&strKmk, &pKmk, &num) < 0 ) [
	if( A205KmkmstSelKey(&strKmk, &pKmk, &num) < 0 ) {
		sFncPutLog ( TRCKIND_WARNING, __FILE__, "AbunInsChk", __LINE__,
			EXEFLG_NON, MYAPP_LOG_ERRCODE_NON, 
			"SELECT Rngchkmst[SQLCODE=%d]", ret ) ;
		return( ret );
	...
	//取得関数を呼び出す。
	//YOON 2007-03-01
	if( A205CalckmkmstSelectKey2(&strCal, &pCal, &num) < 0 ) {
		sFncPutLog ( TRCKIND_WARNING, __FILE__, "AbunInsChk", __LINE__,
			EXEFLG_NON, MYAPP_LOG_ERRCODE_NON, 
...
//		if( !memcmp((pCal+i)->zaicd, strCal.zaicd, 4) &&
//			!memcmp((pCal+i)->sstcd, strCal.sstcd, 7) ) 
//		{
//			iHit = 1;
//			memset(buf, 0, sizeof(buf));
...
//		}
		...
		while(1) {
			//if( !memcmp((pCal+i)->kmkcd, strCal.kmkcd, 7) &&
				//!memcmp((pCal+i)->knsgrp, strCal.knsgrp, 16) &&
			if( !memcmp((pCal+i)->zaicd, strCal.zaicd, 4) &&
				!memcmp((pCal+i)->sstcd, strCal.sstcd, 7) ) {
				iHit = 1;
				memset(buf, 0, sizeof(buf));
			...
			if(iHit == 1 ) {
				break;
			} else if( !memcmp(strCal.sstcd, "0000000", 7) ) {
				break;
			}
			iHit = 0;
			memcpy(strCal.sstcd, "0000000", 7);
		}
	...
	Kenorder *pOrder, int iLine, int Nflg)
{
	...
	int	iIriCnt;
	int		iKetasu;
	char	*pId2;				/* 2009-02-20 緊急SUN EOSL対応 */
	int		iId2pos;			/* 2009-02-20 緊急SUN EOSL対応 */
	long	ret;
	...
	//自動発生項目マスタチェック
	if( (lRet = A205JdkmkSelKey(&strJdk, &pJdk, &iCnt)) < 0 ) {
	...
	for( i=0; i < iCnt ;++i ) {
		...
		// add 2007.05.31 waj 緊急依頼ロットNoを依頼から取得・設定
		pOrder->kkirilotno = pIrai->kkirilotno;
...
//	g_iKmk = iKmk;
	...
	if( A205JdkmkSelAll(&strJdkmk, &iJdkmk) < 0 ) {
//		if( g_strSysknr != NULL ) {
//			ZbmlFree(g_strSysknr);
//			g_strSysknr = NULL;
//		}
...
int insWsseqknri( char *sSriymd, char *sKnsgrp, char *sTrid, int iKntno, int iMinSeq, int iMaxSeq )
{
	...
	if( (lRet = A205wsAllSel( &pWsSeq, &num ) ) < 0 ) {
	...
	if( (lRet = A205wsInsert( &strWsSeq )) < 0 ) {
		sFncPutLog ( TRCKIND_WARNING, __FILE__, "insWsseqknri", __LINE__,
			EXEFLG_NON, MYAPP_LOG_ERRCODE_NON,
