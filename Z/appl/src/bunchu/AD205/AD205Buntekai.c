...
/*  4.00    2009-01-07      Merits K.Ameno  施設コード増桁 通信仕様変更対応 */
...
int		iFLcnt;
{
	...
	char 	tmpiraino[12] = {0,};
	TRAY *pTray;
	RACK *pRack;			/* 2009-02-20 緊急SUN EOSL対応 */
	char ialloc, iPoint;
	int		iWs, iEda;
...
//YOON 2007-02-17
	...
	int		iKmkCd;				/* 2009-01-07 64項目以上対応 追加 */
	int		iMotokenFlg;		/* 2009-02-20 緊急SUN EOSL対応 */
	char	cId2[11];			/* 2009-02-20 緊急SUN EOSL対応 */
...
/*
						sFuncPutErrlist(pst_Env, sFLname, iFLcnt, strBunchuTB.sriymd, 
							...
							"", "枝番間違い WS[%s]", buf);
*/
						// 2007.08.24 INTEC DEL 枝番としてロットNoを付加しているため、チェックを外す
						//iSkipFlg = 1;
						//ZdbRollback();
						//return( 11 );
					}
...
/* 2009-01-07 項目コード 64項目以上対応 INS START */
	//依頼情報コピー
...
/* 2009-01-07 項目コード 64項目以上対応 INS END */
...
/* 2009-01-07 項目コード 64項目以上対応 UPD START */
	for( i=0; i < iKmkCd ;++i ) {
		iOdrMkFlg = iOldFlg;
		pKmkCd1 = (pIrai2+i)->kmkcd;
/* 2009-01-07 項目コード 64項目以上対応 UPD END */
		memset(buf2, 0, sizeof(buf2));
...
//		}
		...
		// del 2007.05.31 waj bgroup==3の時にもオーダを作成するため
		/*
		//YOON 2007-02-01
		if( ret == 3 ) {
			iOdrMkFlg = 1;				//オーダがなし
		...
		*/
		...
		if( (pIrai+k)->odrssflg == ' ' ) {	//依頼が初期状態
			...
			if( iOdrMkFlg == 0 ) {
 			...
 			sFncPutLog ( TRCKIND_DEBUG, __FILE__, "dBunchu_ins", __LINE__,
				EXEFLG_NON, MYAPP_LOG_ERRCODE_NON,
				"依頼重複確認 irino:%s,kmkcd:%s",(pIrai+k)->irino,(pIrai+k)->kmkcd);
			// 複数本取り確認
...
/* 2009-01-07 64項目以上対応 INS START */
					if( pIrai2 != NULL ) {
						ZbmlFree(pIrai2);
						pIrai2 = NULL;
					}
/* 2008-01-07 64項目以上対応 INS END */
					sFncPutLog ( TRCKIND_WARNING, __FILE__, "dBunchu_ins", __LINE__,
						EXEFLG_NON, MYAPP_LOG_ERRCODE_NON,
					...
					if( ret != -803 ) {	//2006-04-23 BUGG FIX
						...
						if( iOdrMkFlg == 0 ) {
							...
							if (( iBgroupFlg == 1 )
								...
								sFncPutLog(TRCKIND_WARNING,__FILE__,"dBunchu_ins",__LINE__,
									EXEFLG_NON, MYAPP_LOG_ERRCODE_NON,
									"INSERT 重複 依頼追加が考えられます。[処理日:%s 検体:%s smpcd:%s kmkcd:%s] [SQLCODE=%d]",
									strBunchuTB.sriymd, strBunchuTB.kntno, strBunchuTB.smpcd, 
 									strBunchuTB.kmkcd, ret );
								break;	/* BUNCHU INSERT ERRORは次へ(項目追加のケースは、再度、同一*/
										/* 検体Noで分注結果がくる為									*/
								/* 								if( pIrai != NULL ) { */
/* 									ZbmlFree(pIrai); */
/* 									pIrai = NULL; */
...
//	if( (!iOdrMkFlg) && (iBgroupFlg == 0) )
	if( ((iOdrMkFlg == 0)|| (iOdrMkFlg == 3))
								&& (iBgroupFlg == 0) ){	
								/* オーダー作成ありまたは重複依頼で、分注エラーの場合はオーダ作成しない */
		...
		if( ialloc == 0 ) {
			...
			*iKenPos += 1;
			...
			{ /* ここで先付情報ファイル格納ディレクトリをラック管理マスタから取得し設定する */
				...
				if( ret = A205rkmstSelectKey(&strRk, &pRk, &num) != 0 ) {
					sFncPutLog ( TRCKIND_WARNING, __FILE__, "dBunchu_ins", __LINE__,
						EXEFLG_NON, MYAPP_LOG_ERRCODE_NON,
