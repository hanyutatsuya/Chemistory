...
static	int		sFncRebind ( struct appEnv * ) ;
...
struct appEnv	*pAppEnv ;
{
	...
	//	ＲＩＢＩＮＤシェル
	if ( ZbmlPrmGetVal ( pAppEnv->bp, MYAPP_PRM_BIN_PATH,
						pAppEnv->cBinPath ) != 0 ) {
		//	異常終了	LOG
		gintErrorCode = MYAPP_STSCODE_ERR_PRM ;
		sFncPutLog ( TRCKIND_ERROR, __FILE__, "ZbmlPrmGetVal", __LINE__,
				 	 EXEFLG_NON, MYAPP_LOG_ERRCODE_NON,
				 	 "パラメータが取得できませんでした[%s]", MYAPP_PRM_BIN_PATH ) ;
		return ( -1 ) ;
	}
	//	ＲＩＢＩＮＤシェル
	if ( ZbmlPrmGetVal ( pAppEnv->bp, MYAPP_PRM_REBIND_SHELL,
						pAppEnv->cRebindShell ) != 0 ) {
		//	異常終了	LOG
		gintErrorCode = MYAPP_STSCODE_ERR_PRM ;
		sFncPutLog ( TRCKIND_ERROR, __FILE__, "ZbmlPrmGetVal", __LINE__,
				 	 EXEFLG_NON, MYAPP_LOG_ERRCODE_NON,
				 	 "パラメータが取得できませんでした[%s]", MYAPP_PRM_REBIND_SHELL ) ;
		return ( -1 ) ;
	}
	//	受信供給
	...
	*/
	for ( li_idx = 0 ; li_idx < MYAPP_RACK_MAXCNT ; li_idx++ ) {
		...
		if ( gtSystemVersion == MYAPP_SYSTEM_OLD ) {
			...
			//	即時チェック情報更新
			li_retSts = sFncUpdREALCHK ( cSel_knskisymd,
										 pst_trInfo->trid,
										 ( 5 * li_idx ) + 1,
										 ( 5 * li_idx ) + 5,
										 pst_trInfo->asyrckid[li_idx], 
			...
			if ( ( recCnt - svRecCnt ) == 0 ) {
				sFncPutLog ( TRCKIND_WARNING, __FILE__, "sFncOutput", __LINE__,
			 	 			 EXEFLG_SEL, MYAPP_LOG_ERRCODE_NON,
			 		 		 "検査オーダ情報、または即時チェック情報が存在しません KNSKISYMD='%s' TRID='%s' RCKID='%s'", cSel_knskisymd, pst_trInfo->trid, pst_trInfo->asyrckid[li_idx] ) ;
			} else {
				svRecCnt = recCnt ;
			}
		}
	...
	for ( idx = 0 ; idx < precCnt ; idx ++ ) {
		...
		if ( strcmp ( SV_asyrckid, precKenorder[idx].asyrckid ) != 0 ||
			...
			sFncCreFileName ( lc_fileName_R, MYAPP_SEND_KIND_CH, 
							  precRealchk[idx].asyrckid, 
							  precRealchk[idx].asyrckpos, 
...
struct my_stREALCHK		*precRealchk ;
{
	...
	int		retSts ;
	//	レコード作成
	retSts = fprintf ( pfp, "%s,%s,%s,%s,%s,%s,%hd,%hd,%s,%s,%s,%hd,%s,%hd,%s,%hd,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%.256s,%.256s,%.256s,%.256s,%.256s,%.256s,%.256s,%.256s,%.256s,%.256s,%.256s,%.256s,%.256s,%.256s,%.256s,%.256s,%.256s,%.256s,%.256s,%.256s,%s\n",
...
{
	long	  ll_retSts ;
	char	  cRebindShellPath[MYAPP_PATH_SIZE];
	pid_t	  pid ;

	/***
	 ***	ＲＥＢＩＮＤシェルパス作成
	 ***/
	sprintf ( cRebindShellPath, "%s/%s", pAppEnv->cBinPath, pAppEnv->cRebindShell ) ;
	...
	if ( pid < 0 ) {
		//	異常終了	LOG
		gintErrorCode = MYAPP_STSCODE_ERR_SHM ;
		sFncPutLog ( TRCKIND_ERROR, __FILE__, "fork", __LINE__,
				 	 EXEFLG_NON, MYAPP_LOG_ERRCODE_NON,
				 	 "子プロセス生成に失敗" ) ;
		return ( -1 ) ;
	}
	if ( pid == 0 ) {
		execl ( cRebindShellPath, pAppEnv->cRebindShell, NULL ) ;
	}
	 ***/
	system ( cRebindShellPath ) ;
